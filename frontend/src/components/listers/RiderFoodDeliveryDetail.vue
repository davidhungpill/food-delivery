<template>
    <v-card outlined>
        <v-card-title>
            FoodDelivery # {{item._links.self.href.split("/")[item._links.self.href.split("/").length - 1]}}
        </v-card-title>
        ...
        <v-card-text>
            ...
            <div>
                <Boolean label="Pickup" v-model="item.pickup" :editMode="editMode" @change="change" />
            </div>
        </v-card-text>
...
<script>
    ...
    export default {
        ...
        async created() {
            ...
            var temp = await axios.get(axios.fixUrl('/foodDeliveries/' + params.id))
            if(temp.data) {
                me.item = temp.data
            }
        },
